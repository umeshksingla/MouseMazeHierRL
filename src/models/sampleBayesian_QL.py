...
class NChainEnv(discrete.DiscreteEnv):
    ...
    def __init__(self, n=5, slip=0.2, small=2, large=10):
        ...
        for s in range(nS - 1):
            P[s][1] = [(slip, s+1, 0., False), (1-slip, 0, small, False)]   # backwards
            P[s][0] = [(slip, 0, small, False), (1-slip, s+1, 0., False)]   # forward and slip
        P[nS - 1][1] = [(slip, nS-1, large, False), (1-slip, 0, small, False)]   # backwards
        P[nS - 1][0] = [(slip, 0, small, False), (1-slip, nS - 1, large, False)]   # forward and slip
...
def normal_gamma_pdf(mu, tau, mu0, lambd, alpha, beta):
    return beta ** alpha * np.sqrt(lambd) / (
                special.gamma(alpha) * np.sqrt(2 * np.pi)) \
           * tau ** (alpha - 0.5) * np.exp(-beta * tau) * np.exp(
        -0.5 * lambd * (mu - mu0) ** 2)


class BQLearning:
    """
    "Bayesian Q-learning". Deadren 1998.
    """
    ...
    def moment_updating(self, state, action, reward, next_state, done):
        ...
        if not done:
            ...
            M2 = reward ** 2 + 2 * self.discount_factor * reward * mean_next + self.discount_factor ** 2 * (
                        ((lamb_next + 1) * beta_next) / (
                            lamb_next * (alpha_next - 1)) + mean_next ** 2)
    ...
    def mixture_updating(self, state, action, reward, next_state, done):
        ...
        if not done:
            ...
            predictive_postetior_tointegrate = self.get_predictive_posterior_tointegrate(
                reward, mean, lamb, alpha, beta, mean_next, lamb_next,
                alpha_next, beta_next)
            ETaufun = lambda mu, tau, x: tau * predictive_postetior_tointegrate(
                mu, tau, x)
            ...
            ELogTaufun = lambda mu, tau, x: np.log(
                tau) * predictive_postetior_tointegrate(mu, tau, x)

            # We have to integrate over mu, tau and x. mu \in [-inf, inf], tau \in [0, \inf], x in [-inf, inf], so it is a triple integral
            ETau = integrate.tplquad(ETaufun, -np.inf, np.inf, lambda a: 0.,
                                     lambda a: np.inf, lambda a, b: -np.inf,
                                     lambda a, b: np.inf)
            EMuTau = integrate.tplquad(EMuTaufun, -np.inf, np.inf, lambda a: 0.,
                                       lambda a: np.inf, lambda a, b: -np.inf,
                                       lambda a, b: np.inf)
            EMu2Tau = integrate.tplquad(EMu2Taufun, -np.inf, np.inf,
                                        lambda a: 0., lambda a: np.inf,
                                        lambda a, b: -np.inf,
                                        lambda a, b: np.inf)
            ELogTau = integrate.tplquad(ELogTaufun, -np.inf, np.inf,
                                        lambda a: 0., lambda a: np.inf,
                                        lambda a, b: -np.inf,
                                        lambda a, b: np.inf)
            ...
            NG[state][action][1] = 1 / (
                        EMu2Tau - ETau * NG[state][action][0] ** 2)
            NG[state][action][2] = max(1.01, self.f(math.log(ETau) - ELogTau,
                                                    num_iterations=20))
    ...
    def get_predictive_posterior_tointegrate(self, r, mean, lamb, alpha, beta,
                                             mean2, lamb2, alpha2, beta2):
        ...
        def predictive_postetior_tointegrate(mu, tau, x):
            ...
            return normal_gamma_pdf(mu, tau, new_mean, new_lambda, new_alpha,
                                    new_beta) * \
    ...
    def Q_sampling_action_selection(self, NG, state):
        ...
        for i in range(self.NUM_ACTIONS):
            ...
            samples[i] = np.random.standard_t(2 * alpha) * np.sqrt(
                beta / (alpha * lamb)) + mean
    ...
    def Myopic_VPI_action_selection(self, NG, state):
        ...
        for i in range(self.NUM_ACTIONS):
            ...
            if i == best_action:
                ranking[i] = c + (mean2 - mean) * getCumulativeDistribution(
                    mean, lamb, alpha, beta, mean2) + mean
            else:
                ranking[i] = c + (mean - means[best_action]) * (
                            1 - getCumulativeDistribution(mean, lamb, alpha,
                                                          beta, means[
                                                              best_action])) + mean
    ...
    def get_c_value(self, mean, lamb, alpha, beta):
        c = math.sqrt(beta) / (
                    (alpha - 0.5) * math.sqrt(2 * lamb) * special.beta(alpha,
                                                                       0.5))
...
def simulate(env_name, num_episodes, len_episode, update_method, selection_method):
    ...
    for episode in range(NUM_EPISODES):
        ...
        for i in range(MAX_T):
            ...
            # Observe the result
            state = obv
            print("state_0, action, reward, state", state_0, action, reward, state)
    ...
    print("Avg Score Phase1:%f| std Phase 1:%f|" % (np.mean(scores1), np.std(scores1)))
    print("=====done=====")
    # print(agent.get_q_function())
    # plt.plot(range(MAX_T), rewardsToGo, label="rewardsToGo")
    # plt.plot(range(MAX_T), rewards, label="rewards")
    # plt.legend()
    # plt.show()
    return scores1, rewardsToGo
...
if __name__ == "__main__":
    ...
    scores_qv_mom, rewardsToGo_qv_mom = simulate(env, n_episodes, horizon, BQLearning.MOMENT_UPDATING, BQLearning.Q_VALUE_SAMPLING)
    # scores_qv_mix = simulate(env, n_episodes, horizon, BQLearning.MIXTURE_UPDATING, BQLearning.Q_VALUE_SAMPLING)
    scores_vpi_mom, rewardsToGo_vpi_mom = simulate(env, n_episodes, horizon, BQLearning.MOMENT_UPDATING, BQLearning.MYOPIC_VPI)
    # scores_vpi_mix = simulate(env, n_episodes, horizon, BQLearning.MIXTURE_UPDATING, BQLearning.MYOPIC_VPI)

    plt.style.use('ggplot')
    fig, ax = plt.subplots()
    ax.plot(cummean(scores_qv_mom), label='QValue sampling - Mom')
    # ax.plot(cummean(scores_qv_mix), label='QValue sampling - Mix')
    ax.plot(cummean(scores_vpi_mom), label='Myopic VPI - Mom')
    # ax.plot(cummean(scores_vpi_mix), label='Myopic VPI - Mix')
    ax.set_xlabel('episode')
    ax.set_ylabel('cumulative average return')
    ax.legend(loc='lower right')
    plt.show()

    plt.plot(range(horizon), rewardsToGo_qv_mom, label="rewardsToGo_qv_mom")
    plt.plot(range(horizon), rewardsToGo_vpi_mom, label="rewardsToGo_vpi_mom")
    plt.ylim((90, 300))
    plt.legend()
    plt.show()
